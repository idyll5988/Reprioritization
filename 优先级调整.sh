...
echo "$date *🔨- 已获取进程$namaapk并调整renice、ionice、taskset和chrt优先级*" 
        # 调整进程优先级
    su -c renice -n -20 -p "$p" || echo "$date *⚠️- renice失败*"
    su -c ionice -c 1 -n 0 -p "$p" || echo "$date *⚠️- ionice失败*"
    su -c taskset -c 0-7 -p "$p" || echo "$date *⚠️- taskset失败*"
    su -c chrt -f -p 1 "$p" || echo "$date *⚠️- chrt失败*"
else
echo "$date *📵- 暗屏状态，跳过优化*"
fi
sleep 10
